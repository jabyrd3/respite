...
class API {
  constructor(){
    ...
    const zone = db.prepare("INSERT INTO domains (name, type, owner) VALUES (?, 'NATIVE', ?);");
    zone.run('example.com', 1);
    zone.run('anotherexample.com', 2);
    const soa = db.prepare("INSERT INTO records (domain_id, name, content, type,ttl,prio) VALUES (?, ?,'localhost admin.example.com 1 10380 3600 604800 3600','SOA',86400,NULL)");
    soa.run(1, 'example.com');
    soa.run(2, 'anotherexample.com');
